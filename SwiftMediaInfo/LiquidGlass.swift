...
extension LinearGradient {
    /// Violet → Pink diagonal
    static let brandVioletPink = LinearGradient(
    ...
    /// Green → Blue diagonal
    static let brandGreenBlue = LinearGradient(
...
private struct GradientCanvas: View {
    ...
    var body: some View {
        Canvas { ctx, size in
            ...
            let t = date.timeIntervalSinceReferenceDate * 0.15
...
struct GlassToolbarButton: ViewModifier {
            ...
            .background {
                if isActive {
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 10, style: .continuous)
                                .strokeBorder(accentColor.opacity(0.4), lineWidth: 0.7)
                        )
                } else {
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
...
extension View {
    func glassToolbarButton(isActive: Bool = false, accentColor: Color = .brandViolet) -> some View {
        modifier(GlassToolbarButton(isActive: isActive, accentColor: accentColor))
    }
}
