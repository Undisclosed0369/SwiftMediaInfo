...
final class MediaStore: ObservableObject {
    ...
    @AppStorage("showAnimatedBackground") var showAnimatedBackground: Bool   = true
    ...
    private func loadInitialFormats(for url: URL, isCompare: Bool) async {
        ...
        // Parse tracks on a background thread to keep UI responsive for large files
        let tracks = await Task.detached(priority: .userInitiated) {
            await MediaEngine.parseTracks(from: json)
        }.value
