...
final class MediaEngine {
    ...
    // MARK: - JSON fetch with live progress reporting for large files
    //
    // Two root causes were fixed in this version:
    //
    // FIX A — --LogFile=/dev/stderr forces progress output in non-TTY mode.
    //   mediainfo only prints "X%" progress lines when it thinks it is attached
    //   to a real terminal (TTY). A GUI app's pipe is NOT a terminal, so mediainfo
    //   stays completely silent by default — that is why the progress bar never
    //   appeared no matter how cleverly we tried to read stderr.
    //   Passing --LogFile=/dev/stderr tells mediainfo to write its internal log
    //   (which includes the percentage lines) explicitly to stderr, regardless of
    //   whether a TTY is present. Without this one flag, nothing works.
    //
    // FIX B — readabilityHandler instead of a polling loop on availableData.
    //   availableData is a one-shot synchronous read: it grabs whatever bytes
    //   are buffered right now and returns immediately with zero bytes if nothing
    //   has arrived yet. The old loop treated zero bytes as "pipe closed" and
    //   exited before mediainfo had written anything. readabilityHandler is the
    //   correct macOS API: the OS invokes our closure every time new bytes become
    //   available on the file descriptor, so we receive data as it arrives and
    //   never exit early.
    //
    // Together, FIX A makes mediainfo actually emit progress, and FIX B makes
    // us reliably receive it.
    
    static func fetchJSONWithProgress(
        _ url: URL,
        onProgress: @escaping (Double) -> Void
    ) async -> String {
        // Bridge the callback-based readabilityHandler into Swift concurrency
        // with a checked continuation. The whole operation runs on a background
        // DispatchQueue so we never block MainActor.
        return await withTaskCancellationHandler {
            await withCheckedContinuation { continuation in
                DispatchQueue.global(qos: .userInitiated).async {
                    ...
                    // FIX A: --LogFile=/dev/stderr tells mediainfo to write its
                    // progress log to stderr even when stdout is a pipe, not a TTY.
                    let args = ["--Output=JSON", "--Full", "--LogFile=/dev/stderr"]
                    ...
                    // FIX B: readabilityHandler is invoked by the OS each time new
                    // bytes arrive on the pipe — no polling, no premature exit.
                    // mediainfo --LogFile log lines look like: "MI_0   0%", "MI_0  47%"
                    // We grab the last whitespace-separated token and strip the "%".
                    var accumulated = ""
                    stderrPipe.fileHandleForReading.readabilityHandler = { handle in
                        let chunk = handle.availableData
                        // Empty data signals the write-end of the pipe was closed.
                        guard !chunk.isEmpty,
                        ...
                        // put it back into the accumulator for the next callback.
                        var lines = accumulated.components(separatedBy: "\n")
                        accumulated = lines.removeLast()
                        for line in lines {
                            let trimmed = line.trimmingCharacters(in: .whitespaces)
                            guard !trimmed.isEmpty else { continue }
                            // The token we want is the last space-delimited word, e.g. "47%"
                            if let token = trimmed.components(separatedBy: .whitespaces).last,
                               token.hasSuffix("%"),
                               let pct = Double(token.dropLast()) {
                    ...
                    // Drain stdout fully (blocking). Safe here because we are on a
                    // background DispatchQueue thread, not the main thread.
                    let stdoutData = stdoutPipe.fileHandleForReading.readDataToEndOfFile()
                    process.waitUntilExit()
                    
                    // Remove the handler now that the process has exited.
...
private extension URL {
    ...
    /// Uses the non-percent-encoded variant on macOS 13+ and falls back to .path.
    var filePath: String {
