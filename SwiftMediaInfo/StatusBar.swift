...
struct StatusBar: View {
    ...
    private func loadingItems(file: MediaFile) -> some View {
        ...
        // Fix #7: Show real progress bar for large files, spinner otherwise
        if let progress = file.analysisProgress {
            HStack(spacing: 8) {
    ...
    private func fileItems(file: MediaFile) -> some View {
        if let general = file.generalTrack {
            ...
            // Fix #4: Use the correct JSON underscore key names instead of slash names.
            // "Duration/String" and "OverallBitRate/String" are TEXT-format keys.
            // In JSON output, mediainfo uses "Duration_String3" / "OverallBitRate_String".
            // We try the richest variant first and fall back progressively.
