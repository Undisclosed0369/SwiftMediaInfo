...
final class MediaStore: ObservableObject {
    ...
    //
    // FIX 3: Run all three fetches IN PARALLEL using async let.
    //
    // Previously they ran one-after-another (sequential). That meant:
    //   - fetchText had to finish before fetchRawText started
    //   - fetchRawText (which uses --Full, so it's slow) had to finish before fetchJSON
    // For some files this pipeline would stall or time out on one step,
    // making the whole file appear broken even though mediainfo could open it fine.
    //
    // Now all three fire at the same time. Whichever finishes first, we store it.
    // The file is marked as fully loaded once ALL three are done.
    // This is both faster AND more reliable for tricky files.
    ...
    private func loadInitialFormats(for url: URL, isCompare: Bool) async {
        ...
        async let jsonResult    = MediaEngine.fetchJSON(url)
